...
static ssize_t show_swipe_enable(struct device *dev, char *buf)
{
	...
	ret += snprintf(buf + ret, PAGE_SIZE, "%d %d\n",
			PAY_TYPE_SWIPE_L, ts->swipe[SWIPE_L].enable);
	ret += snprintf(buf + ret, PAGE_SIZE, "%d %d\n",
			PAY_TYPE_SWIPE_R, ts->swipe[SWIPE_R].enable);
...
{
	struct touch_core_data *ts = to_touch_core(dev);
	int value = (ts->swipe[SWIPE_R2].enable) && (ts->swipe[SWIPE_L2].enable);
	int ret = 0;
		...
		const char *buf, size_t count)
{
	...
	if (enable) {
		...
		ts->swipe[SWIPE_R2].area.x1 = start_x;
		ts->swipe[SWIPE_R2].area.y1 = start_y;
		ts->swipe[SWIPE_R2].area.x2 = end_x;
		ts->swipe[SWIPE_R2].area.y2 = end_y;
		ts->swipe[SWIPE_R2].start_area.x1 = start_x;
		ts->swipe[SWIPE_R2].start_area.y1 = start_y;
		ts->swipe[SWIPE_R2].start_area.x2 = end_x;
		ts->swipe[SWIPE_R2].start_area.y2 = end_y;

		ts->swipe[SWIPE_L2].area.x1 = start_x;
		ts->swipe[SWIPE_L2].area.y1 = start_y;
		ts->swipe[SWIPE_L2].area.x2 = end_x;
		ts->swipe[SWIPE_L2].area.y2 = end_y;
		ts->swipe[SWIPE_L2].start_area.x1 = start_x;
		ts->swipe[SWIPE_L2].start_area.y1 = start_y;
		ts->swipe[SWIPE_L2].start_area.x2 = end_x;
		ts->swipe[SWIPE_L2].start_area.y2 = end_y;
	}

	ts->swipe[SWIPE_R2].enable = (bool)enable;
	ts->swipe[SWIPE_L2].enable = (bool)enable;
