...
int ili9881h_ice_mode_enable(struct device *dev, int mcu_status)
{
	...
	do {
		...
		ret = ili9881h_ice_reg_read(dev, ILI9881H_PID_ADDR, &pid, sizeof(pid));
		if (ret)
...
void ili9881h_dma_clear(struct device *dev)
{
	TOUCH_I("%s\n", __func__);
...
int ili9881h_dma_read_int_flag(struct device *dev)
{
	int retry = 500;
	u32 data = 0;

	do {
		if (ili9881h_ice_reg_read(dev, INTR1_ADDR & BIT(25), &data, sizeof(u32)) < 0)
...
void ili9881h_dma_write(struct device *dev, u32 start, u32 end, u32 len)
{
	ili9881h_ice_reg_write_bit_mask(dev, FLASH0_ADDR, FLASH0_REG_PRECLK_SEL, 1 << 16);
	ili9881h_ice_reg_write(dev, FLASH0_REG_FLASH_CSB, 0x00, 1);
	ili9881h_ice_reg_write(dev, FLASH1_REG_FLASH_KEY1, 0x66aa55, 3);
	ili9881h_ice_reg_write(dev, FLASH2_REG_TX_DATA, 0x0b, 1);

	if (ili9881h_dma_read_int_flag(dev) < 0) {
		TOUCH_E("Write 0xb timeout \n");
		return;
	}

	ili9881h_ice_reg_write_bit_mask(dev, INTR1_ADDR, INTR1_REG_FLASH_INT_FLAG, (1 << 25));
	ili9881h_ice_reg_write(dev, FLASH2_REG_TX_DATA, (start & 0xFF0000) >> 16, 1);
	...
	ili9881h_ice_reg_write_bit_mask(dev, INTR1_ADDR, INTR1_REG_FLASH_INT_FLAG, (1 << 25));
	ili9881h_ice_reg_write(dev, FLASH2_REG_TX_DATA, (start & 0x00FF00) >> 8, 1);
	...
	ili9881h_ice_reg_write_bit_mask(dev, INTR1_ADDR, INTR1_REG_FLASH_INT_FLAG, (1 << 25));
	ili9881h_ice_reg_write(dev, FLASH2_REG_TX_DATA, (start & 0x0000FF), 1);
	...
	ili9881h_ice_reg_write_bit_mask(dev, INTR1_ADDR, INTR1_REG_FLASH_INT_FLAG, (1 << 25));
	ili9881h_ice_reg_write_bit_mask(dev, FLASH0_ADDR, FLASH0_REG_RX_DUAL, 0 << 24);
	ili9881h_ice_reg_write(dev, FLASH2_REG_TX_DATA, 0x00, 1);
	...
	ili9881h_ice_reg_write_bit_mask(dev, INTR1_ADDR, INTR1_REG_FLASH_INT_FLAG, (1 << 25));
	ili9881h_ice_reg_write(dev, FLASH3_REG_RCV_CNT, len, 4);
}

void ili9881h_dma_clear_reg_setting(struct device *dev)
{
	/* 1. interrupt t0/t1 enable flag */
	ili9881h_ice_reg_write_bit_mask(dev, INTR32_ADDR, INTR32_REG_T0_INT_EN | INTR32_REG_T1_INT_EN, (0 << 24));

	/* 2. clear tdi_err_int_flag */
	ili9881h_ice_reg_write_bit_mask(dev, INTR2_ADDR, INTR2_TDI_ERR_INT_FLAG_CLEAR, (1 << 18));

	/* 3. clear dma channel 0 src1 info */
	ili9881h_ice_reg_write(dev, DMA49_REG_DMA_CH0_SRC1_ADDR, 0x00000000, 4);
	ili9881h_ice_reg_write(dev, DMA50_REG_DMA_CH0_SRC1_STEP_INC, 0x00, 1);
	ili9881h_ice_reg_write_bit_mask(dev, DMA50_ADDR, DMA50_REG_DMA_CH0_SRC1_FORMAT | DMA50_REG_DMA_CH0_SRC1_EN, BIT(31));

	/* 4. clear dma channel 0 trigger select */
	ili9881h_ice_reg_write_bit_mask(dev, DMA48_ADDR, DMA48_REG_DMA_CH0_TRIGGER_SEL, (0 << 16));
	ili9881h_ice_reg_write_bit_mask(dev, INTR1_ADDR, INTR1_REG_FLASH_INT_FLAG, (1 << 25));

	/* 5. clear dma flash setting */
	ili9881h_dma_clear(dev);
}

void ili9881h_dma_trigger_reg_setting(struct device *dev, u32 reg_dest_addr, u32 flash_start_addr, u32 copy_size)
{
	int retry = 30;
	u32 stat = 0;

	/* 1. set dma channel 0 clear */
	ili9881h_ice_reg_write_bit_mask(dev, DMA48_ADDR, DMA48_REG_DMA_CH0_START_CLEAR, BIT(25));

	/* 2. set dma channel 0 src1 info */
	ili9881h_ice_reg_write(dev, DMA49_REG_DMA_CH0_SRC1_ADDR, 0x00041010, 4);
	ili9881h_ice_reg_write(dev, DMA50_REG_DMA_CH0_SRC1_STEP_INC, 0x00, 1);
	ili9881h_ice_reg_write_bit_mask(dev, DMA50_ADDR, DMA50_REG_DMA_CH0_SRC1_FORMAT | DMA50_REG_DMA_CH0_SRC1_EN, BIT(31));

	/* 3. set dma channel 0 src2 info */
	ili9881h_ice_reg_write_bit_mask(dev, DMA52_ADDR, DMA52_REG_DMA_CH0_SRC2_EN, (0 << 31));

	/* 4. set dma channel 0 dest info */
	ili9881h_ice_reg_write(dev, DMA53_REG_DMA_CH0_DEST_ADDR, reg_dest_addr, 3);
	ili9881h_ice_reg_write(dev, DMA54_REG_DMA_CH0_DEST_STEP_INC, 0x01, 1);
	ili9881h_ice_reg_write(dev, DMA54_ADDR, DMA54_REG_DMA_CH0_DEST_FORMAT | DMA54_REG_DMA_CH0_DEST_EN, BIT(31));

	/* 5. set dma channel 0 trafer info */
	ili9881h_ice_reg_write(dev, DMA55_REG_DMA_CH0_TRAFER_COUNTS, copy_size, 4);
	ili9881h_ice_reg_write_bit_mask(dev, DMA55_ADDR, DMA55_REG_DMA_CH0_TRAFER_MODE, (0 << 24));

	/* 6. set dma channel 0 int info */
	ili9881h_ice_reg_write_bit_mask(dev, INTR33_ADDR, INTR33_REG_DMA_CH0_INT_EN, (1 << 17));

	/* 7. set dma channel 0 trigger select */
	ili9881h_ice_reg_write_bit_mask(dev, DMA48_ADDR, DMA48_REG_DMA_CH0_TRIGGER_SEL, (1 << 16));

	/* 8. set dma flash setting */
	ili9881h_dma_write(dev, flash_start_addr, (flash_start_addr+copy_size), copy_size);

	/* 9. clear flash and dma ch0 int flag */
	ili9881h_ice_reg_write_bit_mask(dev, INTR1_ADDR, INTR1_REG_DMA_CH0_INT_FLAG | INTR1_REG_FLASH_INT_FLAG, BIT(17) | BIT(25));
	ili9881h_ice_reg_write_bit_mask(dev, 0x041013, BIT(0), 1);
	...
	while (retry > 0) {
		if (ili9881h_ice_reg_read(dev, INTR1_ADDR, &stat, sizeof(u32)) < 0) {
	...
	/* CS High */
	ili9881h_ice_reg_write(dev, FLASH0_REG_FLASH_CSB, 0x1, 1);
...
int ili9881h_gesture_ctrl(struct device *dev, int tci_mode, bool swipe_enable)
{
	struct touch_core_data *ts = to_touch_core(dev);
	int ret = 0;
	u8 data[3] = {0, }, dir = 0;
...
int ili9881h_gesture_failreason_ctrl(struct device *dev, u8 tci_type, u8 swipe_type)
{
	int ret = 0;
	u8 data[6] = {0, };
...
void ili9881h_get_trans_touch_status(struct device *dev)
{
	u8 data[6] = {0};
	...
	ili9881h_dump_packet(data, 8, sizeof(data), 0, "Transfer coodination");
...
int ili9881h_grip_suppression_ctrl(struct device *dev, u16 x, u16 y)
{
	int ret = 0;
	u8 data[5] = {0};
...
int ili9881h_check_int_status(struct device *dev, bool high)
{
	struct touch_core_data *ts = to_touch_core(dev);
...
int ili9881h_check_cdc_busy(struct device *dev, int count, int delay)
{
	...
	if (ret < -1) {
		...
		ili9881h_get_pc_latch(dev);
	}

	return ret;
}

void ili9881h_get_pc_latch(struct device *dev)
{
	u32 pc = 0, latch = 0;
	...
	if (ili9881h_ice_mode_enable(dev, MCU_STOP) < 0)
		TOUCH_E("Enable ice mode failed while reading pc counter\n");

	if (ili9881h_ice_reg_read(dev, ILI9881H_PC_ADDR, &pc, sizeof(u32)) < 0)
		TOUCH_E("Read pc conter error\n");

	if (ili9881h_ice_reg_read(dev, ILI9881H_LATCH_ADDR, &latch, sizeof(u32)) < 0)
		TOUCH_E("Read latch error\n");

	TOUCH_E("read pc (addr: 0x%x) = 0x%x, latch (addr: 0x%x) = 0x%x\n",
		ILI9881H_PC_ADDR, pc, ILI9881H_LATCH_ADDR, latch);

	if (ili9881h_ice_mode_disable(dev) < 0)
...
static int ili9881h_chip_init(struct device *dev)
{
	...
	/* Get Chip ID */
	ret = ili9881h_ice_reg_read(dev, ILI9881H_PID_ADDR, &PIDData, sizeof(PIDData));
	if (ret) {
...
static int ili9881h_sw_reset(struct device *dev, int mode)
{
	...
	if(mode == SW_RESET) {
		...
		ili9881h_ice_reg_write(dev, ILI9881H_IC_RESET_ADDR, ILI9881H_IC_RESET_KEY, 4);
	} else {
...
int ili7807_get_fw_data_wrapper(struct device *dev, u8 *wdata, int wsize, void *rdata, int rsize) {
	...
	/* pre cmd */
	ret = ili9881h_reg_write(dev, CMD_READ_DATA_CTRL, &temp, 1);


	/* cmd */
	ret = ili9881h_reg_write(dev, wdata[0], ptr, wsize);
	if (ret < 0) {
	...
	ret = ili9881h_reg_read(dev, CMD_NONE, rdata, rsize);
	if (ret < 0) {
	...
	ili9881h_dump_packet(rdata, 8, rsize, 16, str);
...
int ili9881h_ic_info(struct device *dev)
{
	...
	ili9881h_get_trans_touch_status(dev);
...
static int ili9881h_lpwg_control(struct device *dev, int tci_mode, bool swipe_enable)
{
	struct touch_core_data *ts = to_touch_core(dev);
	struct ili9881h_data *d = to_ili9881h_data(dev);
	int ret = 0;
	...
	if (tci_mode >= LPWG_NONE && tci_mode <= LPWG_PASSWORD_ONLY) {
		TOUCH_I("ili9881h_lpwg_control - tci_mode = %s, swipe_enable = %s\n",
	...
	if (tci_mode || swipe_enable) {
		ret = ili9881h_gesture_ctrl(dev, tci_mode, swipe_enable);
		ret = ili9881h_gesture_failreason_ctrl(dev, d->tci_debug_type, d->swipe_debug_type);
...
static int ili9881h_deep_sleep_ctrl(struct device *dev, int state)
{
	...
	if (!atomic_read(&ts->state.incoming_call)) { /* IDLE status */
		if (state == IC_DEEP_SLEEP) { // IC_DEEP_SLEEP
			...
			ili9881h_ice_reg_write(dev, ILI9881H_IC_RESET_GESTURE_ADDR,
					ILI9881H_IC_RESET_GESTURE_KEY, sizeof(u32));
			ili9881h_reset_ctrl(dev, HW_RESET_ONLY);
			...
			ili9881h_ice_mode_enable(dev, MCU_ON);
			while (timeout > 0) {
				ili9881h_ice_reg_read(dev, ILI9881H_IC_RESET_GESTURE_ADDR, &value, sizeof(value));

				if (value == ILI9881H_IC_RESET_GESTURE_RUN) {
			...
			ili9881h_gesture_ctrl(dev, ts->lpwg.mode,
				ts->swipe[SWIPE_U].enable || ts->swipe[SWIPE_L].enable || ts->swipe[SWIPE_R].enable);
			ili9881h_gesture_failreason_ctrl(dev, d->tci_debug_type, d->swipe_debug_type);
			ili9881h_switch_fw_mode(dev, FIRMWARE_GESTURE_MODE);
		}
		...
		if (state == IC_DEEP_SLEEP) { // IC_DEEP_SLEEP
			...
			touch_interrupt_control(ts->dev, INTERRUPT_ENABLE);
			atomic_set(&ts->state.sleep, IC_NORMAL);
			ili9881h_lpwg_control(dev, ts->lpwg.mode,
				(ts->swipe[SWIPE_U].enable || ts->swipe[SWIPE_L].enable || ts->swipe[SWIPE_R].enable));
...
int ili9881h_report_tci_fr_buffer(struct device *dev)
{
	struct ili9881h_data *d = to_ili9881h_data(dev);
	struct touch_core_data *ts = to_touch_core(dev);
	int ret = 0, i;
	...
	ret = ili9881h_reg_read(dev, CMD_NONE, tci_fr_buffer, sizeof(tci_fr_buffer));
	if (ret < 0) {
		TOUCH_E("Failed to read buffer type fail reason\n");
	}

	ili9881h_dump_packet(tci_fr_buffer, 8, sizeof(tci_fr_buffer), 16, "buffer type");
...
int ili9881h_report_swipe_fr_buffer(struct device *dev)
{
	struct ili9881h_data *d = to_ili9881h_data(dev);
	struct touch_core_data *ts = to_touch_core(dev);
	int ret = 0, i;
	...
	ret = ili9881h_reg_read(dev, CMD_NONE, swipe_fr_buffer, sizeof(swipe_fr_buffer));
	if (ret < 0) {
		TOUCH_E("Failed to read buffer type fail reason\n");
	}

	ili9881h_dump_packet(swipe_fr_buffer, 8, sizeof(swipe_fr_buffer), 16, "buffer type");
...
static int ili9881h_lpwg_mode(struct device *dev)
{
	...
	if (atomic_read(&ts->state.fb) == FB_SUSPEND) {
		...
		if (ts->lpwg.screen) {
			...
			if(d->tci_debug_type & FAIL_BUFFER_TYPE)
				ili9881h_report_tci_fr_buffer(dev); // Report fr before touch IC reset
			if(d->swipe_debug_type & FAIL_BUFFER_TYPE)
				ili9881h_report_swipe_fr_buffer(dev); // Report fr before touch IC reset
...
static int ili9881h_probe(struct device *dev)
{
	...
	/* Get flash size info */
	ret = ili9881h_read_flash_info(dev);
	if (ret) {
...
//-------------------------------------------------
	...
	ret = ili9881h_fw_upgrade(dev, fwpath);
	if (ret)
...
static int ili9881h_init(struct device *dev)
{
	...
	ili9881h_grip_suppression_ctrl(dev, 15, 400);
...
int ili9881h_switch_fw_mode(struct device *dev, u8 mode)
{
	...
	if (mode == FIRMWARE_DEMO_MODE || mode == FIRMWARE_DEBUG_MODE) {
		...
		ili9881h_dump_packet(mp_code, 8, codeLength, 0, "MP overlay info");
		...
		ili9881h_dma_clear_reg_setting(dev);

		TOUCH_I("[Move ANDES.INIT to DRAM]\n");
		ili9881h_dma_trigger_reg_setting(dev, 0, mp_flash_addr, mp_andes_init_size);

		ili9881h_dma_clear_reg_setting(dev);

		TOUCH_I("[Move MP.TEXT to DRAM]\n");
		ili9881h_dma_trigger_reg_setting(dev, overlay_end_addr, (mp_flash_addr + overlay_start_addr), mp_text_size);

		ili9881h_dma_clear_reg_setting(dev);

		/* Code reset */
		ili9881h_ice_reg_write(dev, 0x040040, 0xAE, 1);
		...
		if (ili9881h_check_cdc_busy(dev, 300, 50) < 0) {
...
static int ili9881h_irq_uart(struct device *dev, u32 len, void *data)
{
	struct ili9881h_data *d = to_ili9881h_data(dev);
	u8 *buf;
	int type = 0;
	...
	ret = ili9881h_reg_read(dev, CMD_NONE, &d->debug_info.uart[len], uart_len);
	if (ret) {
...
static int ili9881h_irq_abs(struct device *dev)
{
	...
	if (d->touch_info.packet_id == I2CUART_PACKET_ID) {
		ili9881h_irq_uart(dev, sizeof(d->touch_info), &d->touch_info);
		return ret;
	}
...
int ili9881h_irq_lpwg(struct device *dev)
{
	...
	ili9881h_dump_packet(data, 8, sizeof(d->gesture_info), 8, "Gesture");

	check_sum = ili9881h_calc_data_checksum(&d->gesture_info, (sizeof(d->gesture_info) - 1));
	...
	switch (pid) {
		...
		case I2CUART_PACKET_ID:
			ili9881h_irq_uart(dev, sizeof(d->gesture_info), &d->gesture_info);
			break;
		default:
...
int ili9881h_irq_debug(struct device *dev) 
{
	...
	if (d->touch_info.packet_id == I2CUART_PACKET_ID) {
		ili9881h_irq_uart(dev, DEBUG_MODE_PACKET_HEADER_LENGTH, &d->debug_info.data);
		return ret;
	}
...
static ssize_t store_grip_suppression(struct device *dev, const char *buf, size_t count)
{
	...
	ret = ili9881h_grip_suppression_ctrl(dev, value_x, value_y);
	if (ret < 0)
...
{
	struct ili9881h_data *d = to_ili7807g_data(dev);
	struct ili9881h_panel_extra_info *panel_extra_info = &d->tp_info.panel_extra_info;
...
static int __init touch_device_init(void)
{
	...
	if (is_lcm_name("MANTIX-ILI9881H")) {
		TOUCH_I("%s, ILI9881H found!!lcm_name = %s\n",__func__,lge_get_lcm_name());
