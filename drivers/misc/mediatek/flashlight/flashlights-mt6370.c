...
#if (CONFIG_LGE_PM_MT6370_FLISHLIGHT_SCENARIO_TIMEOUT > 0)
static struct delayed_work mt6370_work_scenario;
static unsigned int mt6370_scenario_timeout_ms = 1000
		* CONFIG_LGE_PM_MT6370_FLISHLIGHT_SCENARIO_TIMEOUT;
#endif
...
static int torchLevel = 0; //[LGE_UPDATE] [kyunghun.oh@lge.com] [2018-11-02] enable the setting of front flash torch level
...
#if defined(CONFIG_MACH_MT6765_EH35LM)
static const unsigned char mt6370_torch_level_CH1[MT6370_LEVEL_TORCH] = {
	0x00, 0x00, 0x01, 0x02, 0x03, 0x04, /*0x05*/0x04, 0x06, 0x07, 0x08,
...
#elif defined(CONFIG_MACH_MT6762_EH15LM)
static const unsigned char mt6370_torch_level_CH1[MT6370_LEVEL_TORCH] = {
	0x00, 0x00, 0x01, 0x02, 0x03, 0x04, /*0x05*/0x08, 0x06, 0x07, 0x08,
	0x10, 0x0A, 0x0B, 0x0C, 0x0D, 0x0E
};
...
static const unsigned char mt6370_torch_level_CH1[MT6370_LEVEL_TORCH] = {
	0x00, 0x02, 0x04, 0x06, 0x08, 0x08, 0x08, 0x16, 0x10, 0x12,
	0x14, 0x16, 0x18, 0x1A, 0x1C, 0x1E
};
...
#elif defined(CONFIG_MACH_MT6765_DH40LM)
static const unsigned char mt6370_torch_level_CH1[MT6370_LEVEL_TORCH] = {
	0x00, 0x02, 0x04, 0x06, 0x08, 0x0A, 0x0A, 0x0E, 0x10, 0x12,
	0x14, 0x16, 0x18, 0x1A, 0x1C, 0x1E
};
...
static const unsigned char mt6370_torch_level_front_for_app[MT6370_LEVEL_TORCH] = {
	0x00, 0x00, 0x01, 0x02, 0x03, 0x04, 0x05, 0x06, 0x07,
	0x08, 0x09, 0x0A, 0x0B, 0x0C, 0x0D
};
static const unsigned char mt6370_torch_level_rear_for_app[MT6370_LEVEL_TORCH] = {
	0x00, 0x00, 0x00, 0x00, 0x00, 0x04, 0x04, 0x04, 0x04,
	0x04, 0x09, 0x09, 0x09, 0x09, 0x0E
};
...
/*LGE_CHANGE_S, 2020-05-15, enabled dual flash path for EH35/MH45, seonyung.kim@lge.com*/
#if defined(CONFIG_MACH_MT6762_MH45LM) || defined(CONFIG_MACH_MT6765_EH35LM)
		mt6370_en_ch2 = mt6370_en_ch1;
#endif
/*LGE_CHANGE_E, 2020-05-15, enabled dual flash path for EH35/MH45, seonyung.kim@lge.com*/
	pr_err("enable(%d,%d), mode:%d.\n",
...
#if (CONFIG_LGE_PM_MT6370_FLISHLIGHT_SCENARIO_TIMEOUT > 0)
			cancel_delayed_work(&mt6370_work_scenario);
			schedule_delayed_work(&mt6370_work_scenario,
					msecs_to_jiffies(mt6370_scenario_timeout_ms));
#endif
		}
	} else {
		if (is_decrease_voltage) {
...
/*LGE_CHANGE_S, 2018-09-19, Implemented Flash Torch level for LGE senario, soojong.jin@lge.com*/
static int mt6370_set_torch_level(int level, int channel)
{
    if(level > 14)
        level = 14;
    if (!mt6370_is_torch(level)) {
		if (channel == MT6370_CHANNEL_CH1){
			printk("[LGE] set rear flash torch level(IDX) = %d\n", level);
			flashlight_set_torch_brightness(
					flashlight_dev_ch1, mt6370_torch_level_rear_for_app[level]);
			torchLevel = level;
		}
		else if (channel == MT6370_CHANNEL_CH2) {
			printk("[LGE] set front flash torch level(IDX) = %d\n", level);
			flashlight_set_torch_brightness(
					flashlight_dev_ch2, mt6370_torch_level_front_for_app[level]);
			torchLevel = level;
		}
		else {
			pr_err("other channel - default set\n");
			torchLevel = level;
		}
    }
...
/*LGE_CHANGE_E, 2018-09-19, Implemented Flash Torch level for LGE senario, soojong.jin@lge.com*/

#if (CONFIG_LGE_PM_MT6370_FLISHLIGHT_SCENARIO_TIMEOUT > 0)
static void mt6370_work_disable_scenario(struct work_struct *data)
{
	if (!is_decrease_voltage)
		return;

	pr_info("flashlight scenario timeout (%ums)\n",
			mt6370_scenario_timeout_ms);
	mt6370_set_scenario(
...
static int mt6370_ioctl(unsigned int cmd, unsigned long arg)
{
	...
	/*LGE_CHANGE_S, 2018-09-19, Implemented Flash Torch level for LGE senario, soojong.jin@lge.com*/
	case FLASH_IOC_SET_FLASH_TORCH_LEVEL:
		pr_info("[LGE] FLASH_IOC_SET_TORCH_LEVEL(%d) Channel(%d)\n", fl_arg->arg, channel);
		fl_arg->arg = mt6370_set_torch_level(fl_arg->arg, channel); //LGE_CHANGE_S, Implemented Flash Torch level for each channel, 2019-02-14, seungmin.hong@lge.com
		break;
	/*LGE_CHANGE_E, 2018-09-19, Implemented Flash Torch level for LGE senario, soojong.jin@lge.com*/
...
#if (CONFIG_LGE_PM_MT6370_FLISHLIGHT_SCENARIO_TIMEOUT > 0)
	INIT_DELAYED_WORK(&mt6370_work_scenario,
			mt6370_work_disable_scenario);
#endif
...
static int mt6370_remove(struct platform_device *pdev)
{
	...
	/*LGE_CHANGE_S, Implemented Flash Torch level for each channel, 2019-02-14, seungmin.hong@lge.com*/
	mt6370_set_torch_level(0, MT6370_CHANNEL_CH1); /*LGE_CHANGE, 2018-09-19, Implemented Flash Torch level, soojong.jin@lge.com*/
	mt6370_set_torch_level(0, MT6370_CHANNEL_CH2); /*LGE_CHANGE, 2018-09-19, Implemented Flash Torch level, soojong.jin@lge.com*/
	/*LGE_CHANGE_E, Implemented Flash Torch level for each channel, 2019-02-14, seungmin.hong@lge.com*/
