...
static atomic_t g_pwm_is_clock_on = ATOMIC_INIT(-1);
	...
	if (!led_node) {
		...
		if (g_pwm_led_mode == MT65XX_LED_MODE_NONE) {
			...
			if (!ret) {
				...
				led_mode = MT65XX_LED_MODE_CUST_BLS_PWM;
				g_pwm_led_mode = led_mode;
#endif
	...
	enum disp_pwm_id_t id, int enabled)
{
	...
	if (enabled) {
		...
		disp_pwm_set_drverIC_en(id, enabled);
#if defined(CONFIG_MTK_AAL_SUPPORT)
		chargepump_set_backlight_level(0);
#endif
	}

	PWM_MSG("PWM REG Dump in %s", __func__);
	PWM_MSG("== START: DISP PWM%d REGS ==", index);
	PWM_MSG("(0x000)PWM_EN			=0x%x", DISP_REG_GET(reg_base + DISP_PWM_EN_OFF));
	PWM_MSG("(0x008)PWM_CON_0		=0x%x", DISP_REG_GET(reg_base + DISP_PWM_CON_0_OFF));
	PWM_MSG("(0x010)PWM_CON_1		=0x%x", DISP_REG_GET(reg_base + DISP_PWM_CON_1_OFF));
	PWM_MSG("(0x028)PWM_DEBUG		=0x%x", DISP_REG_GET(reg_base + 0x28));
	PWM_MSG("-- END: DISP PWM%d REGS --", index);
}
...
static int disp_pwm_level_remap(enum disp_pwm_id_t id, int level_1024)
{
	...
	if (read_done_flag == 0) {
		...
		PWM_ERR("%s : lge_boot_reason : 0x%x \n",__func__, lge_boot_reason);
		read_done_flag = 1;
	}

	if (!fb_blank_called && (level_1024 != 0)) {
		if(lge_boot_reason == LGE_REBOOT_REASON_LCD_OFF){
			level_1024 = 8;
			PWM_ERR("%s : lcd dimming mode! minimum brightness (%d). \n", __func__,level_1024);
		}
	}
#endif
	return level_1024;
	...
	int level_1024, void *cmdq)
{
	...
	if (atomic_read(&g_pwm_is_clock_on) == 0) {
			PWM_NOTICE("%s: primary display is slept, only pwm will power on", __func__);
			primary_display_pwm_power_on();
			is_suspend = true;
	}
	if ((DISP_PWM_ALL & id) == 0) {
	...
	if(is_suspend == true)  {
                PWM_NOTICE("%s: complete. primary display is slept, pwm will power off", __func__);
                primary_display_pwm_power_off();
    }
#endif
	return 0;
}
...
static int ddp_pwm_power_on(enum DISP_MODULE_ENUM module, void *handle)
{
	...
	atomic_set(&g_pwm_is_clock_on, 1);
...
static int ddp_pwm_power_off(enum DISP_MODULE_ENUM module, void *handle)
{
	...
	atomic_set(&g_pwm_is_clock_on, 0);
...
void set_pwm_backlight_brightness(int level_1024)
{
	disp_bls_set_backlight(level_1024);
}

int get_pwm_backlight_brightness(void)
{
	int old_pwm = 0;
	int index = 0;

	index = index_of_pwm(disp_pwm_get_main());

	old_pwm = atomic_read(&g_pwm_backlight[index]);
