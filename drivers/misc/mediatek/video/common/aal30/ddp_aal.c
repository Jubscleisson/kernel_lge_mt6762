...
static int cur_ess_sth_index = UI_60_PER_ESS_STR_INDEX; // default setting
#define ESS_INDEX_MASK          (0xFFFF)
#define INDEX_OF_ESS_STH(value) (value & ESS_INDEX_MASK)
#endif
#endif
...
#if defined (AAL_SUPPORT_KERNEL_API) && defined (CONFIG_LGE_USE_ESS_STRENGTH_TABLE)
int set_ess_strength_by_brightness(int bl_level)
{
  int new_ess_sth_index = cur_ess_sth_index;

  if(bl_level < 0){
    goto done;
  }

#ifdef CONFIG_LGE_11BIT_BRIGHTNESS_TABLE
  if (bl_level <= UI_90_PER && bl_level > UI_80_PER){
    new_ess_sth_index = UI_60_PER_ESS_STR_INDEX;
  } else if (bl_level <= UI_80_PER && bl_level > UI_60_PER) {
    new_ess_sth_index = UI_60_PER_ESS_STR_INDEX;
  } else if (bl_level <= UI_60_PER && bl_level > UI_40_PER) {
    new_ess_sth_index = UI_40_PER_ESS_STR_INDEX;
  } else if (bl_level <= UI_40_PER && bl_level > UI_30_PER) {
    new_ess_sth_index = UI_30_PER_ESS_STR_INDEX;
  } else if (bl_level <= UI_30_PER && bl_level > UI_20_PER) {
    new_ess_sth_index = UI_20_PER_ESS_STR_INDEX;
  } else if (bl_level <= UI_20_PER && bl_level > UI_10_PER) {
    new_ess_sth_index = UI_10_PER_ESS_STR_INDEX;
  } else if (bl_level <= UI_10_PER) {
    new_ess_sth_index = MIN_ESS_STR_INDEX;
  } else { // bl_level > UI_90_PER
    new_ess_sth_index = UI_60_PER_ESS_STR_INDEX;
  }
#else
  if (bl_level <= UI_90_PER && bl_level > UI_80_PER){
  ...
  cur_ess_sth_index = new_ess_sth_index;

  AAL_DBG("cur ess strength : %d\n",cur_ess_sth_index);

done :
  return new_ess_sth_index;
}

void  disp_aal_set_ess_strength(int bl_level, int ess_strength)
{
  unsigned long flags;
  int level_command = 0;
  int set_ess_sth_index = UI_60_PER_ESS_STR_INDEX; // default ess strength

  set_ess_sth_index = set_ess_strength_by_brightness(bl_level);

  AAL_DBG("%s : set - %d, cur - %d",__func__, set_ess_sth_index, cur_ess_sth_index);

  AAL_DBG("(get)g_aal_ess_level_cmd_id = %d, level = %d (cmd = 0x%x)(%d)",
            INDEX_OF_ESS_STH(g_aal_ess_level_cmd_id), INDEX_OF_ESS_STH(set_ess_sth_index), INDEX_OF_ESS_STH(level_command), INDEX_OF_ESS_STH(level_command));

  spin_lock_irqsave(&g_aal_hist_lock, flags);

  g_aal_ess_level_cmd_id += 1;
  g_aal_ess_level_cmd_id = g_aal_ess_level_cmd_id % 64;
  level_command = AAL_CONTROL_CMD(g_aal_ess_level_cmd_id, set_ess_sth_index);

  AAL_DBG("(set)g_aal_ess_level_cmd_id = %d, level = %d (cmd = 0x%x)(%d)",
            INDEX_OF_ESS_STH(g_aal_ess_level_cmd_id), INDEX_OF_ESS_STH(set_ess_sth_index), INDEX_OF_ESS_STH(level_command), INDEX_OF_ESS_STH(level_command));
  ...
  AAL_DBG("(set)level = %d (cmd = 0x%x)(%d)", INDEX_OF_ESS_STH(set_ess_sth_index), INDEX_OF_ESS_STH(level_command), INDEX_OF_ESS_STH(level_command));
}
#endif
...
#if defined (AAL_SUPPORT_KERNEL_API) && defined (CONFIG_LGE_USE_ESS_STRENGTH_TABLE)
	disp_aal_set_ess_strength(bl_1024,g_aal_hist.essStrengthIndex);
#endif
...
#if defined (AAL_SUPPORT_KERNEL_API) && defined (CONFIG_LGE_USE_ESS_STRENGTH_TABLE)
	AAL_DBG("essStrengthIndex = %d",INDEX_OF_ESS_STH(g_aal_hist.essStrengthIndex));
#endif
