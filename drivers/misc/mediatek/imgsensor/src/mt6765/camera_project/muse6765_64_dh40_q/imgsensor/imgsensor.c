...
#define PDAF_DATA_SIZE 8392
extern m290_otg_data_t m290_otp_data[2];
...
/*M290 OTP bring up*/
m290_otg_data_t m290_otp_data[2];
char *otp_content[] = {"XTC_1", "XTC_2", "SensorXTC", "PDXTC_1", "PDXTC_2"};
kal_uint16 m290_otp_read(otp_format_t *p, u16 i2c_addr, enum OTP_FORM otp_form, UINT8* data_c)
{
...
//	int i = 0;
	...
	data_c[M290_OTP_SIZE - 2 - otp_form] = (((checksum % 0xFFFF)+check) == checksum_read);
	printk("m290_otp_read : (%d %s) group(%d) checksum(0x%04x) checksum_read(0x%x) check_ok2(%d)",
			otp_form, otp_content[otp_form], group, (checksum % 0xFFFF) + check, checksum_read, data_c[M290_OTP_SIZE - 2 - otp_form]);
#endif
#if 0
	for (i = 0; i < p->size - 5; i += 6)
		printk("m290_otp_read data %d: 0x%02x, 0x%02x 0x%02x 0x%02x 0x%02x 0x%02x", i, data[i], data[i+1], data[i+2], data[i+3], data[i+4], data[i+5]);
	while (i < p->size) {
		printk("m290_otp_read data %d: 0x%02x ", i, data[i]);
		i++;
	}
#endif
	return 0;
...
#define STRINGIZE(stringizedName) TOSTRING(stringizedName)
	...
	static bool rear_cam = false, front_cam = false, back2_cam = false, 
				back3_cam = false, front2_cam = false; //, camer_ok = false;
	...
	for (i = 0; i < MAX_NUM_OF_SUPPORT_SENSOR; i++) {
		...
		if (i == 1) {
		        drv_idx = psensor->inst.sensor_idx+5; // orderedSearchList[i];
		} else if (i > 1) {
		        pr_err("ERROR:NULL this sensor [%d]\n", i);
		        break;
		}
		...
		if (pSensorList[drv_idx].init) {
			...
			if (psensor->pfunc) {
				...
				if (!imgsensor_check_is_alive(psensor)) {
                    /*M510,M710 add I2C info for runintest APP*/
...
static ssize_t imgsensor_name_show(struct device *dev, struct device_attribute *attr, char *buf)
{
    ...
    struct IMGSENSOR_SENSOR* sensor = imgsensor_sensor_get_inst(IMGSENSOR_SENSOR_IDX_MAIN);
    num1 = sprintf(buf, "back_main: %s\n", sensor->inst.psensor_name);

    sensor = imgsensor_sensor_get_inst(IMGSENSOR_SENSOR_IDX_SUB);
    num2 = sprintf(buf + num1, "front: %s\n", sensor->inst.psensor_name);

    sensor = imgsensor_sensor_get_inst(IMGSENSOR_SENSOR_IDX_MAIN2);
    num3 = sprintf(buf+num1+num2, "back2: %s\n", sensor->inst.psensor_name);

    sensor = imgsensor_sensor_get_inst(IMGSENSOR_SENSOR_IDX_SUB2);
    num4 = sprintf(buf+num1+num2+num3, "back3: %s\n", sensor->inst.psensor_name);

    sensor = imgsensor_sensor_get_inst(IMGSENSOR_SENSOR_IDX_MAIN3);
    sprintf(buf+num1+num2+num3+num4, "back4: %s\n", sensor->inst.psensor_name);

    ret = strlen(buf) + 1;
    return ret;
}

static DEVICE_ATTR(imgsensor_name, 0664, imgsensor_name_show, NULL);
static struct kobject * sensor_kobject;

UINT8 fusion_id[20];
UINT8 fusion_id_front[20];
...
static ssize_t sensorid_show(struct device *dev, struct device_attribute *attr, char *buf)
{
    ...
    num = 6;
    for (i = 0; i < 10; i++) {
		sprintf(buf + num + 2*i, "%02x", fusion_id[i]);
    }
    ret = strlen(buf) + 1;
    strncpy(buf + ret, "\nfront: ", 9);
    for (i = 0; i < 10; i++) {
		sprintf(buf + ret + 9 + 2*i, "%02x", fusion_id_front[i]);
    }
	strncpy(buf + ret + 10 + 2*i, "\n", 2);
...
static int __init imgsensor_init(void)
{
    ...
    sensor_kobject = kobject_create_and_add("android_imgsensor", NULL);
    if (sensor_kobject == NULL) {
        pr_info("[imgsensor_init]Big error: sensor_kobject_create_sysfs_ failed\n");
    } else {
        ret = sysfs_create_file(sensor_kobject, &dev_attr_imgsensor_name.attr);
