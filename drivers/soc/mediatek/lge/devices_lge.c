...
char *rev_str[] = {
	...
	"rev_0_1",
	"rev_a",
#if defined(CONFIG_MACH_MT6765_DH40LM)
	"rev_a_1",
#endif
	"rev_b",
	"rev_c",
...
char* lge_get_board_revstr(void)
{
	return rev_str[lge_bd_rev];
}
...
static int __init lcm_name_setup(char *value)
{
    ...
    if(value != NULL) {
	    if (strlen(value) < sizeof(lcm_name)) {
		    strncpy(lcm_name, value, strlen(value));
		    printk("[Touch] lcm_name: %s\n", lcm_name);
	    } else {
		    printk("[Touch] buffer oveflow of value(lcm_name) : strlen(value) = %zu/ sizeof(value) = %zu\n",
...
static int __init device_name_setup(char *value)
{
	...
	if (value != NULL) {
	    if (strlen(value) < sizeof(device_name)) {
		strncpy(device_name, value, strlen(value));
		printk("[Touch] device_name: %s\n", device_name);
	    } else {
		    printk("[Touch] buffer oveflow of value(device_name) : strlen(value)= %zu/ sizeof(value) = %zu\n",
				    strlen(value), sizeof(value));
	    }
	} else {
...
#if defined(CONFIG_LGE_EXTAMPTYPE_HWINFO)
static int lge_extamptype_hwinfo = 0;
static int __init lge_extamptype_hwinfo_setup(char *extspkamp)
{
	if (!strcmp(extspkamp, "1")) {
		lge_extamptype_hwinfo = 1;
		printk("LGE use RT5509 \n");
	} else if (!strcmp(extspkamp, "0")) {
		lge_extamptype_hwinfo = 0;
		printk("LGE use AW87318 \n");
	} else {
		lge_extamptype_hwinfo = 0;
		printk("Undefined speaker amp. default is AW87318\n");
	}
	return 1;
}
__setup("androidboot.vendor.lge.extspkamp=", lge_extamptype_hwinfo_setup);

int lge_get_extamptype_hwinfo(void)
{
	return lge_extamptype_hwinfo;
}
#endif
